...
import { getWeeklyStats, getUserProjectStats } from '@/lib/api/services/dashboard';
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const promises: Promise<any>[] = [
        getWeeklyStats(user.id, weekStartDate),
      ...
      if (canApproveTimesheets) {
        promises.push(
          timesheetsService.getAll({ team: 'true', status: 'pending' }).then((ts) => ts.data.length)
        );
      }
      ...
      const projectStatsResponse = results[1] as import('@kairos/shared').UserProjectStats;
      setProjectStats(projectStatsResponse.projects || []);
      const holidaysResponse = results[2] as import('@kairos/shared').HolidaysResponse;
